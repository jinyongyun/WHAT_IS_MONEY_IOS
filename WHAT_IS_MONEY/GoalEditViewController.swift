...
class GoalEditViewController: UIViewController, UINavigationControllerDelegate & UIImagePickerControllerDelegate {
    ...
    // MARK: - [URL Session Post 멀티 파트 사진 데이터 업로드]
       func requestPOST() {
           ...
           // MARK: [URL 지정 실시]
           let urlComponents = URLComponents(string: "https://www.pigmoney.xyz//goal/uploadGoalImage/\(goalIdx ?? 0)/\(userIdx)")
    ...
    func convertImageToBase64(image: UIImage) -> String {
        ...
        func patchGoal() {
            ...
            let patchgoal = patchgoal(amount: Int(PriceTextField.text ?? "5") ?? 5, category_name: GoalNameTextField.text ?? "알 수 없음", init_amount: Int(GoalPriceTextField.text ?? "5") ?? 5)
